	...
	<div class="searchUser">
		...
		<div v-if="type == 'name'">
			<div class="row">
				<div class="col-md-8">
					<div class="form-group">
						 <input type="email" class="form-control" id="name" placeholder="Search With Word" v-model="search" @keyup="() => {data = []; page = 1; this_page = 0;}">
					</div>
				</div>
				<div class="col-md-4">
					<button type="button" class="btn btn-secondary btn-block" @click="install">Search</button>
				</div>
			</div>
		</div>
	...
	export default {
		...
		methods:{
			install() {
				...
				axios.post('/api/admin/post/'+ vm.type + '?page=' + vm.page,{search:vm.search}).then(response => {
					// Last Page Make 
